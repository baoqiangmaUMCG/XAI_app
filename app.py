...
def run_eval(model_state: Dict[str, Any], cache_tuple, eval_kind: str, manual_target, preds_json_text, gt_file, is_nifti_mask: bool, slice_idx: int):
    ...
    # Determine the class to evaluate: manual > used_target from Explain > 0
    try:
        t = int(manual_target)
    except Exception:
        t = -1
    ...
    if eval_kind in ("Pointing Game", "Mask ROC-AUC"):
        if gt_file is None:
...
with gr.Blocks(title="XAI + Evaluation (Medical imaging ready)") as demo:
    ...
    with gr.Tab("1) Model"):
        ...
        with gr.Row():
            with gr.Column():
                #model_src = gr.Radio(["Demo DenseNet121 (ImageNet)", "Upload TorchScript .pt"], label="Model source", value="Demo DenseNet121 (ImageNet)")
                model_src = gr.Radio(
                         ["Demo DenseNet121 (ImageNet)", "Torchvision Model", "Upload TorchScript .pt"],
                         label="Model source",
                ...
                classmap_file = gr.File(label="Optional: class map JSON (e.g., [\"cat\", \"dog\", ...])", file_types=[".json"], visible=True)
                load_btn = gr.Button("Load model", variant="primary")
                model_info  = gr.Textbox(label="Model info (JSON)", lines=10)
        ''' 
        def toggle_model_src(src):
            return gr.update(visible=(src == "Upload TorchScript .pt"))

        model_src.change(toggle_model_src, inputs=model_src, outputs=uploaded_model)
        '''
        def toggle_model_src(src):
          return (
        ...
        model_src.change(
            toggle_model_src,
            inputs=model_src,
            outputs=[uploaded_model, torchvision_model_name]
        )


        def do_load_model(src, f, cmap, tv_name):
        ...
        load_btn.click(do_load_model, inputs=[model_src, uploaded_model, classmap_file, torchvision_model_name], outputs=[model_state, model_info])
    ...
    with gr.Tab("3) Evaluate"):
        with gr.Row():
            with gr.Column():
                eval_kind = gr.Dropdown(["Deletion AOPC", "Insertion AOPC", "Pointing Game", "Mask ROC-AUC"], value="Deletion AOPC", label="Evaluation")
        ...
        eval_btn.click(
            run_eval,
            inputs=[model_state, cache_tuple, eval_kind, manual_target, preds_json, gt_file, is_nifti_mask, slice_idx2],
